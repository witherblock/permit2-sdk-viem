...
export abstract class AllowanceTransfer {
  ...
  public static getPermitSingleData(permit: PermitSingle, permit2Address: Address, chainId: number) {
  ...
  // return the data to be sent in a eth_signTypedData RPC call
  // for signing the given permit data
  public static getPermitBatchData(permit: PermitBatch, permit2Address: Address, chainId: number) {
    invariant(MaxSigDeadline >= permit.sigDeadline, 'SIG_DEADLINE_OUT_OF_RANGE')
  ...
  public static hash(permit: PermitSingle | PermitBatch, permit2Address: Address, chainId: number): string {
    if (isPermit(permit)) {
      const { domain, values } = AllowanceTransfer.getPermitSingleData(permit, permit2Address, chainId)
      ...
      const { domain, values } = AllowanceTransfer.getPermitBatchData(permit, permit2Address, chainId)
